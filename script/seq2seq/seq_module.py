...
class Seq2seq(object):
    ...
    def build_graph(self, is_training):
        ...
        if is_training:
            self.iter = batch_seq2seq(hp.datapath)
            self.x, self.y, self.z, self.x_lengths, self.y_lengths, self.tag = self.iter.get_next()
        ...
        with tf.variable_scope("embedding"):
            ...
            tag_emb = tf.get_variable('lookup_table_tag',
                                           dtype=tf.float32,
                                           shape=[hp.num_tags, hp.emb_size_tag],
                                           initializer=tf.contrib.layers.xavier_initializer())
            #if zero_pad:
            ...
            self.tag_emb = tf.nn.embedding_lookup(tag_emb, tag)
        ...
        with tf.variable_scope("encoder"):
            ## Embedding
            self.total_enc = tf.concat((self.enc, self.tag_emb), axis=-1)
        ...
        with tf.Session(graph=self.graph) as sess:
            ...
            for epoch in range(start_epoch, hp.num_epochs_main+1):
                ...
                for step in range(num_batch):
                    ...
                    # optimize
                    #if epoch == 1:
                    #    y_pred, z_pred = sess.run([self.y, self.z])
                    #    print(y_pred[0])
                    #    print(z_pred[0])
